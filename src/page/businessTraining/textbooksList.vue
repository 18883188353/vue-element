        ...
        <div class="main">
            ...
            <el-card class="box-card">
                <div slot="header" class="clearfix">
                    <span>教材列表：</span>
                    <!--<el-link type="primary" class="fr mr20" @click="">学习点评</el-link>-->
                    <el-link type="primary" class="fr mr20" @click="knowledgeAdd">编制知识</el-link>
                </div>
                <!--<div class="text item">
                ...
                <el-table
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            {{getBookTypeDesc(scope.row.bookType)}}
                        </template>
                    </el-table-column>
                    <!--<el-table-column
                        prop="status"
...
<script>
    import queryTextbookList from '../../api/other/queryTextbookList'
    import delTextbookOne from '../../api/other/delTextbookOne'
    ...
    export default {
        ...
        methods:{
            ...
            knowledgeAdd(row, type){
                if (row.id){
                    this.$router.push({'path': 'textbooksAdd', query:{
                            id: row.id,
                            type: type
                        }});
                } else {
             ...
             */
            knowledgeAudit(row){
                this.$router.push({'path': 'knowledgeAudit', query:{
                        id: row.id,
                    }})
            },
            /**
            ...
            del(row) {
                this.$confirm('此操作将删除, 是否继续?', '提示', {
                    ...
                    delTextbookOne.call(this, {id: row.id}).then(res =>{
                        if (res.Code === 1){
                            this.$message({
                                type: 'success',
                                message: '删除成功!'
            ...
            getBookTypeDesc(status){
                let _desc = '';
                this.bookTypeOptions.map(item =>{
                    if (item.value === status) {
                        _desc = item.label;
                    }
                });
            ...
            searchList(){
                ...
                queryTextbookList.call(this,_postData).then(res => {
                    if (res.Code === 1){
