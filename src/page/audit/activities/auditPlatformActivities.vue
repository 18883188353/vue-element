...
export default {
    ...
    data() {
        return {
            ...
            queryCondition: { // 查询条件
                taskTypeName: ['PlatformActivitiesAudit'], // 审核任务名称
    ...
    // keep-alive 组件激活时调用，如果不走缓存请使用其他生命周期钩子，例如mounted
    activated() {
        this.init();
    ...
    methods: {
        ...
        auditCheckInDetails(row) {
            this.$router.push({ 'path': `auditPlatformActivitiesDetails/${row.taskId}` })
        }
    },
