            ...
            >
                ...
                >
                    <template slot-scope="scope">
                        <span v-if="!scope.row.ArticleId || scope.row.ArticleId === '0' ">0</span>
                        <span v-else>{{ scope.row.ArticleId.split(',').length }}</span>
                    </template>
                </el-table-column>
                <el-table-column
                    prop="Manager"
...
export default {
    ...
    data() {
        return {
            taskTypeNames: [{
                ...
                name: '全球购',
                code: 'GlobalShopBookAudit'
            }, {
                name: '奢侈品',
                code: 'LuxuryShopBookAudit'
            }],
            queryCondition: { // 查询条件
                taskTypeName: ['ShopBookAudit', 'GlobalShopBookAudit', 'LuxuryShopBookAudit'], // 审核任务名称
    ...
    methods: {
        ...
        auditCheckInDetails(row) {
            this.$router.push({ 'path': `auditArticleDetails/${row.taskId}` })
        }
