                ...
                <el-row>
                    <el-col :span="12">
                        <el-form
                            ...
                            class="mt20"
                        >
                            <el-form-item label="营业执照号：">{{ beforeData.LicenseCard }}</el-form-item>
                            <el-form-item label="营业执照：">
                                <viewer :images=[beforeData.LicenseImg]>
                                    <img
                                        :src="beforeData.LicenseImg"
                                        style="width: 150px; height: 200px;"
                                    />
                                </viewer>
                            </el-form-item>
                            <el-form-item label="成立日期：">{{ beforeData.LicenseCreateTime }}</el-form-item>
                            <el-form-item label="有效期至：">{{ beforeData.IsEver == 1 ? '永久有效' : beforeData.LicenseValidTime === '0001-01-01' ? '': beforeData.LicenseValidTime }}</el-form-item>
                        </el-form>
                    </el-col>
                    <el-col :span="12">
                        <el-form
                        ...
                        >
                            <el-form-item
                                :class="beforeData.LicenseCard === afterData.LicenseCard ? '' : 'redItem'"
                                label="营业执照号："
                            >{{ afterData.LicenseCard }}</el-form-item>
                            <el-form-item
                                :class="beforeData.LicenseImg  === afterData.LicenseImg  ? '' : 'redItem'"
                                label="营业执照："
                            >
                                <viewer :images=[afterData.LicenseImg]>
                                    <img
                                        :src="afterData.LicenseImg"
                                        style="width: 150px; height: 200px;"
                                    />
                                </viewer>
                            </el-form-item>
                            <el-form-item
                                :class="beforeData.LicenseCreateTime === afterData.LicenseCreateTime ? '' : 'redItem'"
                                label="成立日期："
                            >{{ afterData.LicenseCreateTime }}</el-form-item>
                            <el-form-item
                                :class="beforeData.IsEver === afterData.IsEver ? (beforeData.LicenseValidTime === afterData.LicenseValidTime ? '' : 'redItem') : 'redItem'"
                                label="有效期至："
                            >{{ afterData.IsEver == 1 ? '永久有效' : afterData.LicenseValidTime === '0001-01-01' ? '': afterData.LicenseValidTime }}</el-form-item>
                        </el-form>
                    </el-col>
                </el-row>
                <el-row v-if="isAudit">
        ...
        init() {
            AuditDetail.call(this, { taskId: this.id }).then(res => {
                if (res.Code === 1) {
                    ...
                    this.beforeData = entityInfo.businessLicenseInfoBefore ? entityInfo.businessLicenseInfoBefore : {};
                    this.afterData = entityInfo.businessLicenseInfoAfter ? entityInfo.businessLicenseInfoAfter : {};
