                    ...
                    <div class="onlineservrce">
                        ...
                        <div class="box_left">
                            <div class="user">
                                ...
                                <div class="search">
                                    <el-input size="mini" placeholder="请输入关键词搜索" v-model="search"><i slot="prefix" class="el-input__icon el-icon-search"></i></el-input>
                                </div>
                            </div>
                            <div class="user_list">
                                <vue-scroll>
                                    <ul>
                                        ...
                                        <li :class="isUserShow === i ? 'active' : '' " v-for="(item,i) in leftUserList" :key="i" @click="changePersonBtn(item.To_Account,i,item.C2cNick)">
                                            <img :src=item.C2cImage alt="">
                                            <p>
                                                <em>{{item.C2cNick}} <i v-if="item.noRead">{{item.noRead ? item.noRead : ''}}</i></em>
                                                <span>{{item.MsgShow}}</span>
                                                <i>{{item.MsgTimeStamp ? $Tool.formatDate(item.MsgTimeStamp*1000, 'hh:mm') : ''}}</i>
                                            </p>
                                        </li>

<!--                                        <li :class="isUserShow === i ? 'active' : '' " v-for="(item,i) in leftUserList" :key="i" @click="changePersonBtn(item.to_account,i,item.from_account_name)">-->
<!--                                            <img :src="item.from_account_head" alt="">-->
<!--                                            <p>-->
<!--                                                <em>{{item.from_account_name}} <i v-if="item.noRead">{{item.noRead ? item.noRead : ''}}</i></em>-->
<!--                                                <span>{{item.new_message}}</span>-->
<!--                                                <i>{{(item.msg_time).substring(11,18)}}</i>-->
<!--                                            </p>-->
<!--                                        </li>-->
                        ...
                        <div class="box_main">
                            ...
                            <div class="chat_content">
                                <div class="chat_srcoll">
                                    <!--<div class="moreMsg" @click="moreMsgBtn">查看更多历史消息</div>-->
                                    <div v-for="(item,i) in historyRecord" :key="i" :class="item.id === userInfo.tencentUser.tx_account ? 'right' : 'left' ">
                                        <div class="time">
                                            <p v-if="listTime(item.time, i)">
                                                <span>{{listTime(item.time, i)}}</span>
                                            </p>
                                        </div>
                                        <div class="content">
                                            <div class="word">
                                                ...
                                                <div class="custom_cont" v-if="item.custom">
                                                    <div class="custom">
                                                        <div class="pro_img"><img :src='item.custom.ImgUrl' alt=""></div>
                                                        <div class="details">
                                            ...
                                            <div class="lt"></div>
                                            <div class="touxiang">
<!--                                                <img :src="item.id !== userInfo.tencentUser.tx_account ? objectInformation.C2cImage : require('../../assets/img/mj.png')" alt="">-->
                                                <img src="../../assets/img/mj.png" alt="">
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="chat_input">
                                <vue-scroll>
                                    <div class="">
                                        <el-input type="textarea" :rows="2" placeholder="请输入内容" v-model="myMessage" :autosize="{ minRows: 3, maxRows: 6}" maxlength="120" show-word-limit  @keyup.enter.native="toSend()"></el-input>
                                    </div>
                                </vue-scroll>
                                <div class="tr mt5"><el-button type="primary" size="medium" @click="toSend()">发送</el-button></div>
                            </div>
                        </div>
                        <!--右侧用户信息-->
                        <div class="box_right">
    ...
    export default {
        ...
        data(){
            return {
                ...
                pageNum: 1,//聊天页码
                pageSize: 10,//聊天页码
                type: 1,//0：查询指定用户的所有聊天记录（包含和其他客服的聊天记录） 1：只查询当前客服和指定用户的聊天记录（to_account必传）
                moreMsg: "",//查看更多历史消息
                recentList: [],// 最近联系人列表
                search: '',//搜索
                myMessage: '',//发送消息内容
        ...
        methods:{
            ...
            onMsgNotify(newMsgList) {
                ...
                newMsgList.forEach(async msg => {
                    if (msg.fromAccount === this.custIdentifier || msg.fromAccount === this.userInfo.tencentUser.tx_account) {   // 新消息来自当前聊天对象或自己
                        ...
                        if (this.leftUserList.every(res => res.To_Account !== msg.fromAccount)) { // 新消息不属于联系人列表
                        ...
                        const obj = this.leftUserList.find(res => res.To_Account === msg.fromAccount) || {}
            ...
            getRecentList() {
                ...
                // }).then(res => {
                //     if (res.Code === 1) {
                //         this.leftUserList = res.Data.list
                //     } else {
                //
                //     }
                // })
            ...
            changePersonBtn(o,i,name){
                ...
                this.getHistory();
                //切换聊天对象时，消息记录条数清除
                const obj = this.leftUserList.find(res => o === res.To_Account) || {}
                obj.noRead = 0
             ...
             * 根据返回的type id等信息拼接至chatlist聊天窗口去
             */
            showMessage(msg) {
                let id = msg.fromAccount;
...
<style>
    .chat_input /deep/ .el-textarea__inner{border: 1px solid #fff;}
    .chat_input /deep/ .el-textarea__inner:hover{border-color: #fff;}
</style>
